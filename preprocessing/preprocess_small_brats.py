"""
Preprocess the SmallBrats dataset (Dataset017) for training and save to nnUNet_preprocessed directory.

This script:
1. Loads the dataset split from Dataset017_SmallBrats
2. Preprocesses ALL training samples (from the single split)
3. Saves preprocessed data to a new directory: Dataset017_SmallBrats_Fast
...
def load_dataset_split(dataset_path: Path) -> Dict:
    """Load the dataset split from dataset directory."""
    splits_file = dataset_path / "splits_final.json"
...
def main():
    ...
    # Paths
    raw_data_path = Path("/ministorage/ahb/data/nnUNet_raw/Dataset017_SmallBrats")
    preprocessed_base = Path("/ministorage/ahb/data/nnUNet_preprocessed")
    output_dir = preprocessed_base / "Dataset017_SmallBrats_Fast"
    ...
    splits = load_dataset_split(raw_data_path)
    
    # Get all training cases from all folds (should be just one fold)
    all_train_cases = []
    for fold_data in splits:
        all_train_cases.extend(fold_data['train'])
        all_train_cases.extend(fold_data['val']) # Preprocess validation cases too!
    
    # Remove duplicates and sort
    all_train_cases = sorted(list(set(all_train_cases)))
    
    logger.info(f"Total cases available (Train+Val): {len(all_train_cases)}")
    ...
    if successful_cases:
        ...
        dataset_info = {
            "name": "Dataset017_SmallBrats_Fast",
            "description": "Fast preprocessed SmallBrats dataset for MedNeXt training",
